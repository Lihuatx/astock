...
`astock` 是面向个人研究的 A 股模拟交易系统。通达信 TdxQuant 提供主行情，同花顺 Financial-API 用于盘后补充和交叉校验；系统内部自建 A 股账户、风控、OMS、账本和模拟 Broker。P4 正在增加不可变复盘快照、Windows runner 和私有只读 Dashboard。
